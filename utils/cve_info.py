...
# +++ 请用这个版本替换 utils/cve_info.py 中的 cvemap_search 函数 +++

def cvemap_search(cve, info_dir):
    """
    使用 vulnx 搜索单个CVE的详细信息，并处理其返回的JSON结构。
    """
    cvemap_json_path = os.path.join(info_dir, "cvemap.json")
    
    # 如果文件已存在，直接读取并返回内容，避免重复调用工具
    if os.path.exists(cvemap_json_path) and os.path.getsize(cvemap_json_path) > 0:
        try:
            with open(cvemap_json_path, 'r', encoding='utf-8') as f:
                ...
                if isinstance(json_data, list):
                    # 如果返回的是列表（旧逻辑或某些情况），取第一个元素
                    return json_data[0] if json_data else None
                elif isinstance(json_data, dict):
                    # 如果返回的是字典（vulnx 的情况），直接返回该字典
                    return json_data
                else:
                    # 如果是未知格式，返回 None
                    logger.warning(f"Unexpected data type in {cvemap_json_path}: {type(json_data)}")
                    return None
        except (json.JSONDecodeError, IndexError) as e:
            logger.error(f"Error reading existing cvemap.json for {cve}: {e}. Will re-fetch.")
            # 文件损坏或为空，继续执行后面的逻辑重新获取
    
    # 构造使用 vulnx 的命令
    cvemap_command = f"vulnx search -v {cve} --detailed -j > {cvemap_json_path}"
    
    logger.info(f"Executing command: {cvemap_command}")
    
    try:
        # 执行命令
        subprocess.run(cvemap_command, shell=True, check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, timeout=60)
        ...
        with open(cvemap_json_path, 'r', encoding='utf-8') as f:
            ...
            if isinstance(json_data, list):
                ...
                logger.warning(f"Unexpected data type from new vulnx output for {cve}: {type(json_data)}")
                return None

    except subprocess.CalledProcessError as e:
        logger.error(f"Command failed for CVE {cve}: {e.stderr}")
        return None
    except (FileNotFoundError, json.JSONDecodeError) as e:
        logger.error(f"Failed to process result for CVE {cve}: {e}")
        return None
...
def cve_analysis(cve, output_dir="resources/"):
    """
    分析单个CVE，此版本已适配 vulnx 工具的输出格式。
    """
    cve_dir = f"{output_dir}/{cve}"
    info_dir = f"{cve_dir}/info"
    searching_start_time = time.time()

    # 假设 compose 函数用于创建目录结构
    compose(output_dir, cve)
    if not os.path.exists(info_dir):
        # 根据 compose 的行为，这行可能可以移除，但保留也无害
        os.makedirs(info_dir, exist_ok=True)
    
    # 调用搜索函数获取 vulnx 的原始输出
    ...
    if cvemap_json is None:
        logging.error(f"Failed to retrieve data for {cve} from cvemap_search.")
        searching_time = time.time() - searching_start_time
        return searching_time, 0  # 返回与成功时一致的元组格式，避免后续解包错误

    # 2. 安全地从返回的JSON对象中提取 'results' 列表
    results_list = cvemap_json.get('results')

    # 3. 检查 'results' 列表是否有效（存在、是列表、且不为空）
    if not isinstance(results_list, list) or not results_list:
        logging.error(f"Could not find a valid 'results' list in the data for {cve}.")
        searching_time = time.time() - searching_start_time
        return searching_time, 0

    # 4. 从列表中获取第一个元素，这才是包含CVE详细信息的字典
    cve_details = results_list[0]
    
    # 5. 使用正确的键名 'description' 和安全的 .get() 方法提取漏洞描述
    cve_description = cve_details.get('description', 'No description available.')

    # --- 核心修改逻辑结束 ---

    logging.info(f"Analyzing {cve} with description: '{cve_description[:100]}...'")
    ...
    logging.info(f"Analysis time for {cve}: {analysis_time:.2f} seconds")
    
    features_path = os.path.join(cve_dir, "features.json")
